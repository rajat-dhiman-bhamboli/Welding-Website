...
export default function SignupPage() {
  ...
  return (
    <div className="bg-brand-dark min-h-screen py-32 flex items-center justify-center px-4 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-full glow-bg blur-[150px] opacity-10" />
      
      <div className="glass p-10 rounded-[3rem] border-white/5 w-full max-w-md relative z-10 shadow-2xl">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-black text-white uppercase tracking-tighter">Create <span className="text-brand-orange">Account</span></h1>
          <p className="text-gray-500 mt-2 font-bold uppercase text-[10px] tracking-[0.3em]">Join the Elite Technicians</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button 
            disabled={loading}
            className="w-full py-5 bg-brand-orange text-white font-black rounded-2xl hover:scale-[1.02] transition-all shadow-xl shadow-brand-orange/30 uppercase tracking-[0.2em] text-sm disabled:opacity-50"
          >
            {loading ? "Constructing Account..." : "Sign Up"}
          </button>
        </form>

        <p className="mt-8 text-center text-gray-500 font-bold text-xs uppercase tracking-widest">
          Already have an account? <Link href="/login" className="text-brand-orange hover:underline ml-1">Log In</Link>
        </p>
      </div>
    </div>
  );
